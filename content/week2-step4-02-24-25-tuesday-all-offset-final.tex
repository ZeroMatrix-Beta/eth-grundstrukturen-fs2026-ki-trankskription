...
\lecturechapter{2}{Dienstag}{24. Feb}{24. Februar 2025}{Formale Beweise und Theorien}
...
\begin{axioms}[Aussagenlogische Axiomenschemata]\label[axioms]{ax:aussagenlogisch}
...
In unserem Fall ist $\Phi = \emptyset$, $\psi \equiv x = y$ and $\varphi \equiv y = x$. Da wir gerade $\{x = y\} \vdash y = x$ bewiesen haben, liefert uns das Deduktionstheorem sofort:
...
\varphi_7 &\equiv y = x && \text{(aus \ref{eq:sym-step1} and \ref{eq:sym-step6} via $\text{MP}$)} \label{eq:sym-step7}
\end{align}
...
\begin{math-stroke}[Signatur der Ringtheorie]
...
Die Axiome der Körpertheorie bestehen aus den acht Axiomen der Ringtheorie, die wir hier als $\mathbf{KT_0}$ bis $\mathbf{KT_7}$ bezeichnen, plus drei weiteren Axiomen:
\begin{itemize}
...
\begin{math-stroke}[Die Axiome der Ringtheorie]
...
\subsection{Körpertheorie}

\begin{math-stroke}[Die Axiome der Körpertheorie]
...
\mathbf{KT_8}\colon &\quad \forall x \forall y (x \cdot y = y \cdot x) && \text{(Kommutativität der Multiplikation)} \label{eq:KT8_part3} \\
\mathbf{KT_9}\colon &\quad \forall x \bigl(\neg(x = 0) \rightarrow \exists y (x \cdot y = 1)\bigr) && \text{(Existenz des multiplikativen Inversen)} \label{eq:KT9_part3} \\
\mathbf{KT_{10}}\colon &\quad \neg(0 = 1) && \text{(Nicht-Trivialität)} \label{eq:KT10_part3}
\end{align}
\end{axioms}
\end{math-stroke}
...
\begin{math-stroke}[Beweis der Eindeutigkeit des Inversen]
...
\begin{math-stroke}[Detaillierte formale Schritte]
...
\subsection{Ordnungstheorie}

\begin{math-stroke}[Axiome der Ordnungstheorie]
...
Dieses Axiom besagt anschaulich: Zwei Mengen sind genau dann identisch, wenn sie exakt dieselben Elemente enthalten. Eine Menge ist also vollständig durch ihre Elemente charakterisiert. Es gibt keine \qt{Zusatzinformationen} oder innere Strukturen, die zwei Mengen mit identischen Elementen voneinander unterscheiden könnten. 
...
\begin{math-stroke}[Das Extensionalitätsaxiom]
