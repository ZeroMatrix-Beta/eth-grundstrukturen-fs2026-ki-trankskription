...
\subsection{Axiome 0 bis 3 der Mengenlehre}
...
\subsection{Konstruktion der natürlichen Zahlen nach von Neumann}
\begin{spoken-clean}[continued]
...
\subsection{Unendlichkeitsaxiom und Aussonderungsaxiom}
...
\subsection{Definition des Durchschnitts}
...
\begin{itemize}
    \item Eine funktionale Formel $\varphi(x, y)$ (inklusive möglicher weiterer Parameter $w_1, \dots, w_k$) mit $\forall x \exists! y \, \varphi(x, y)$ nennen wir eine \newterm{Klassenfunktion} $F$. Wir schreiben $y = F(x)$ als Abkürzung für $\varphi(x, y)$.
...
\subsection{Das Fundierungsaxiom (Axiom 8)}
