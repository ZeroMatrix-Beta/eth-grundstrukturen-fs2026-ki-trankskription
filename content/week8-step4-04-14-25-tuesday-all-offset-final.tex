...
\lecturechapter{Dienstag}{14. Apr}{14. April 2025}{Das Auswahlaxiom, Ordinalzahlen und Äquivalenzen}
...
\begin{itemize}
    ...
    \item \textbf{Äquivalenzen des Auswahlaxioms:} Das Kuratowski-Zorn-Lemma (KZL), das Teichmüller-Prinzip (TP) und der vollständige Beweis des Äquivalenzsatzes.
\end{itemize}
\end{nice-box}
...
\begin{didactic-insight}[Notationskonvention des Auswahlaxioms]
...
\begin{definition}[Ordinalzahl]
    \label[definition]{def:ordinalzahl}
...
\par\vspace{1ex}\noindent
\textbf{Beispiele für Ordinalzahlen (Natürliche Zahlen):}
\begin{align*}
0 &:= \emptyset \\
1 &:= \{0\} = \{\emptyset\} \\
...
Unter den Voraussetzungen von \cref{thm:transfinite-rekursion} existiert eine eindeutige Funktion $a \colon \lambda \to A$, sodass für alle $\beta \in \lambda$ gilt:
...
Sei $\lambda = \omega$ and $A = \omega$. Wir definieren die Fibonacci-Folge als Funktion $a \colon \omega \to \omega$ durch:
\begin{align*}
F_0 &= a(0) = 0, \\
F_1 &= a(1) = 1, \\
...
\par\vspace{1ex}\noindent
\textbf{Beweisvorbereitung (Wohlordnung der Vektoren):}
...
\begin{spoken-clean}[00:48:05 - 00:49:55]
...
Folglich ist die obere Schranke $m$ ein maximales Element von $P$. Dies beweist $\text{WOP} \implies \text{KZL}$.
\end{short-proof}
\end{math-stroke}
...
\begin{itemize}
    ...
    \item Da $X_{\max} \in \mathcal{C} \subseteq \mathcal{F}$ and $\mathcal{F}$ endlichen Charakter hat, muss auch die Teilmenge $E \in \mathcal{F}$ sein.
\end{itemize}
...
\mathcal{S} := \left\{ g \subseteq \mathcal{F} \times \bigcup \mathcal{F} \;\middle|\; g \text{ ist eine Funktion, } \operatorname{dom}(g) \subseteq \mathcal{F} \text{ und } \forall X \in \operatorname{dom}(g) \, (g(X) \in X) \right\}.
\end{equation}
Wir zeigen, dass die Familie $\mathcal{S}$ endlichen Charakter besitzt:
\begin{itemize}
    \item Eine Relation $g$ ist genau dann eine Funktion in $\mathcal{S}$, wenn jede endliche Teilmenge von Paaren in $g$ eine eindeutige Zuordnung definiert und die Auswahlbedingung erfüllt. Dies ist eine rein lokale Eigenschaft, die sich auf endliche Teilmengen reduzieren lässt.
\end{itemize}
