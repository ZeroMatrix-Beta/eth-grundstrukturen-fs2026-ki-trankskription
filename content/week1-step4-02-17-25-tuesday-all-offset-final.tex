...
\lecturechapter{1}{Dienstag}{17. Feb}{17. Februar 2026}{Grundstrukturen}
...
\begin{math-stroke}[Ziele der Vorlesung]
...
\begin{math-stroke}[Software-Tools]
\begin{itemize}
...
\begin{math-stroke}[Literatur zur Philosophie der Mathematik]
\begin{itemize}
...
Das Zweite sind logische Operatoren. \inlinemetanote{schreibt an die Tafel} Davon gibt es vier: Es gibt diesen Haken, der heißt \qt{nicht}. Dann gibt es ein Dach nach oben, das ist \qt{und}. Ein nach oben offener Keil, das ist \qt{oder}. Und dann gibt es noch das Zeichen --- einfach ein Pfeil ---, das heißt \qt{impliziert}. 
...
Gut, dann haben wir \textbf{(c)}, das wären die logischen Quantoren. \inlinemetanote{schreibt an die Tafel} Da gibt es zwei: \qt{es existiert} --- das ist der Existenzquantor --- und \qt{für alle} --- der Allquantor. 
...
\begin{itemize}
    ...
    \item Wir bezeichnen mit $\operatorname{frei}(\varphi)$ die Menge aller Variablen, die in $\varphi$ mindestens ein freies Vorkommen besitzen.
\end{itemize}
\end{definition}
\end{math-stroke}
...
Eine Substitution $\varphi(\nu/\tau)$ heißt \newterm{zulässig}, falls keine Variable, die in $\tau$ vorkommt, an einer Stelle für $\nu$ substituiert wird, die im Bereich eines Quantors dieser Variablen liegt (d.\,h. keine Variable in $\tau$ wird durch die Substitution \qt{eingefangen}).
\end{definition}
\end{math-stroke}
