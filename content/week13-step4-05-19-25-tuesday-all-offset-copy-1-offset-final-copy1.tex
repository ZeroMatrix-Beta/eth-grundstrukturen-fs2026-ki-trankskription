...
\lecturechapter{Dienstag}{19. Mai}{19. Mai 2025}{Äquivalenzrelationen modulo \texorpdfstring{$n$}{n}, Diffie-Hellman, Chinesischer Restsatz und RSA}
...
\begin{enumerate}[label=\textbf{(\arabic*)}]
    ...
    \item \textbf{Schwierige Richtung (Diskretes Logarithmusproblem):} Für gegebene Werte $a, x, n \in \mathbb{N}$ ist es rechnerisch extrem aufwendig, den Exponenten $b$ zu bestimmen, sodass gilt:
    \[
    a^b \equiv x \pmod n.
    \]
\end{enumerate}
...
Angenommen, $\operatorname{ggT}(a, n) = 1$. Nach dem Lemma von Bézout existieren ganze Zahlen $b, \ell \in \mathbb{Z}$, sodass gilt:
\[
...
\begin{enumerate}[label=\textbf{(\arabic*)}]
    \item \textbf{Abgeschlossenheit:} Seien $[a], [a'] \in \mathbb{Z}_n^*$. Dann existieren $[b], [b'] \in \mathbb{Z}_n$ mit $[a] \cdot [b] = [1]$ and $[a'] \cdot [b'] = [1]$. Für das Produkt gilt:
    \[
...
\begin{math-stroke}[Der Satz von Euler]
...
\subsection{Historische Einordnung und das Problem von Sunzi}
...
\]
Die Bijektivität von $\Phi$ ist die direkte algebraische Übersetzung von Theorem \ref{thm:chinesischer-restsatz}. Die Homomorphie-Eigenschaft bezüglich Addition und Multiplikation folgt unmittelbar aus der komponentenweisen Definition der Ringoperationen auf dem direkten Produkt:
\[
\Phi([x \cdot y]_M) = \left( [x \cdot y]_{m_1}, \dots, [x \cdot y]_{m_r} \right) = \left( [x]_{m_1}, \dots, [x]_{m_r} \right) \cdot \left( [y]_{m_1}, \dots, [y]_{m_r} \right) = \Phi([x]_M) \cdot \Phi([y]_M).
\]
\end{explanation-of-steps}
\end{math-stroke}
