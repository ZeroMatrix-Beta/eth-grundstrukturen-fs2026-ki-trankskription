...
\lecturechapter{Dienstag}{12. Mai}{12. Mai 2025}{Elementare Zahlentheorie: Euklidischer Algorithmus, Primzahlen und Modularrechnen}
...
Das Problem mit dem $\mathbb{N}$ für die natürlichen Zahlen is, dass nicht klar ist, was gemeint ist. Es ist vielleicht etwa --- ich weiß auch nicht --- die Hälfte der Leute, die sagt, dass die $0$ nicht dazugehört, und die andere Hälfte sagt, dass die $0$ dazugehört. Ich habe da keine starke Meinung, aber es gibt Leute mit sehr starken Meinungen dazu. \inlinemetanote{lacht} Ich mache es aber so, wie es im Skript steht: dass die $0$ mit dabei ist. Das ist so, ja...
...
Okay, dann machen wir das Nächste: den größten gemeinsamen Teiler. Nächste Proposition --- das haben Sie vielleicht auch schon gesehen ---: Wir nehmen $a$ und $b$ als natürliche Zahlen, aber diesmal nicht beide gleich $0$. Machen wir das so. Dann existiert --- um ganz sicher zu sein \inlinemetanote{lacht} --- eine eindeutige natürliche Zahl $d \in \mathbb{N}$, sodass gilt: Erstens, $d \mid a$ und $d \mid b$. Und das Zweite ist: Für alle natürlichen Zahlen $k$, sodass $k \mid a$ und $k \mid b$ gilt, folgt $k \mid d$. Also jeder andere gemeinsame Teiler teilt auch $d$.
...
Relativ direkt, das heißt, das ist alles klar. Also die Aussage ist ja eigentlich auch schon klar, wenn man das so aufschreibt. \inlinemetanote{lacht} Das weiß man auch schon aus der Grundschule, dass der größte gemeinsame Teiler eindeutig ist. Das ist also keine große Überraschung. Ja.
\end{spoken-clean}
...
Nein, das kann man nicht finden, nein. \inlinemetanote{lacht} Weil sonst könnten Sie es voneinander subtrahieren, und dann...
\end{spoken-clean}
...
Äh, ja, muss man ein bisschen überlegen. Da könnte man zum Beispiel... könnte man zum Beispiel $2 \cdot 5 - 3 \cdot 3$... Okay, ja. Es funktioniert tatsächlich immer. \inlinemetanote{lacht}

Okay. Also ich bin nicht ganz überzeugt von den Argumenten, aber sonst... \inlinemetanote{lacht} Es muss so sein, das ist genau der kleinste gemeinsame Teiler. Weil eben gerade... genau, weil es minimal ist. Genau mit dem Argument kann man zeigen: Wenn Sie einen gemeinsamen Teiler finden, dann muss es dieser sein.
\end{spoken-clean}
...
Wir nehmen an, wir haben zwei Primfaktorzerlegungen für eine Zahl $a \in \mathbb{N}_{_{>1}}$:
...
Sei $n \in \mathbb{Z}$ und seien $a, b \in \mathbb{Z}$. Wir sagen, $a$ und $b$ sind \newterm{kongruent modulo $n$}, falls $n$ die Differenz $a - b$ teilt:
\[ n \mid (a - b) \]
